...
export default function ProjectDetailPage() {
  ...
  const votingRef = useRef(false) // prevents double-clicks
  ...
  const handleVote = async () => {
    ...
    if (!profile || !project || votingRef.current) return
    ...
    // Re-fetch actual vote count from DB to stay in sync
    const { data: fresh } = await supabase
      .from('projects').select('votes').eq('id', project.id).single()
    if (fresh) setProject(p => p ? { ...p, votes: fresh.votes } : p)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 flex-wrap mb-6">
        ...
        <button
          onClick={handleVote}
          disabled={!profile || votingRef.current}
          title={!profile ? 'Sign in to vote' : voted ? 'Remove vote' : 'Upvote'}
      ...
      <div className="card p-5">
        ...
        {remarks.length === 0 && (
          <p className="text-sm text-gray-400 mb-4">No remarks yet. Be the first to comment.</p>
        )}
        ...
        {profile ? (
          ...
          <p className="text-sm text-gray-400">
            <a href="/auth/login" className="font-medium underline" style={{ color: 'var(--olive)' }}>
              Sign in
            </a> to leave a remark
          </p>
        )}
