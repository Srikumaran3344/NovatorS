...
import { Clock, CheckCircle, XCircle, ChevronDown, ChevronUp, ExternalLink } from 'lucide-react'
              ...
              <div className="p-5 flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    Submitted {new Date(project.created_at).toLocaleDateString('en-SG', { day: 'numeric', month: 'short', year: 'numeric' })}
                    {' '}· OC/NC: {project.oc_name}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {project.status === 'rejected' && (
                    <Link href={`/submit/edit/${project.id}`}
                      className="text-xs font-medium px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 transition-colors">
                      Resubmit
                    </Link>
                  )}
                  {/* Update for approved */}
                  {project.status === 'approved' && (
                    <Link href={`/submit/edit/${project.id}`}
                      className="text-xs font-medium px-2.5 py-1 rounded-md bg-gray-50 text-gray-700 hover:bg-gray-100 border border-gray-200 transition-colors">
                      Update
                    </Link>
                  )}
                        ...
                        <div key={ev.id} className="flex gap-3">
                          <div className="flex flex-col items-center">
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${
                              ev.to_status === 'approved' ? 'bg-green-100'
                              : ev.to_status === 'rejected' ? 'bg-red-100'
                              : 'bg-blue-50'
                            }`}>
                          ...
                          <div className="pb-2">
                            ...
                            {ev.remarks && (
                              <p className="text-xs text-gray-600 mt-1 bg-white border border-gray-200 rounded px-2 py-1">
                                "{ev.remarks}"
                              </p>
                            )}
